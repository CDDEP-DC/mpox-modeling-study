...
# Parameters ####
param_msm <- function(init.inf = initial_infections,                                # initial number of infected people 
                      ...
                      demography.change = demography.change.switch,
                      num_travelers = num_travelers_seed,
                      ...
                      inf.prob = 0.81,       # probability of infection upon sexual contact
                      percent_asymp = prop_asymp, # prob of inf. person seeking treatment at all
                      risk_reinfection = prop_reinfection, # Risk of reinfection
                      time_wane_from_rec = r_to_s_time,
                      
                      reinfect.switch = reinfections.change.switch,
                      asymp = asymp.change.switch, 
                      init.vacc1 = 0,
                      init.vacc2 = 0,
                      vacc.effect1 = vacEffect1,
                      vacc.effect2 = vacEffect2,
                      vacc.effect.delayedVax = vacEffectdelayedVax,
                      vax_mechanism = type_of_vax,
                      arrival.rate = enteringRate,
                      removal.rate = leavingRate,
                      ...) {
...
# Control Settings ####
control_msm <- function(simno = seq(1,num.nsims, by=1),
                        ...
                        vax.FUN = vax_msm,
                        arrivals.FUN = new_adults,
                        aging.FUN = aging,
                        departures.FUN = departures,
...
initialize_msm <- function(x, param, init, control, s, s.wane, v1, v2) { #So this is what sets up the network that is then changed by simnet
  ...
  dat <- set_attr(dat, "vaccWaneTime_1", vaccWaneTime_1)   # record time step of vaccination
  dat <- set_attr(dat, "vaccWaneTime_2", vaccWaneTime_2)   # record time step of vaccination
  dat <- set_attr(dat, "status", status)       # initial status
  dat <- set_attr(dat, "waned_1", waned_1)       # initial status
  dat <- set_attr(dat, "waned_2", waned_2)       # initial status
...
new_adults <- function(dat, at) {
    demography.change <- get_param(dat, "demography.change")
    
    if(demography.change == TRUE){
    ...
    if (nArrivals > 0) {
      ...
      dat <- append_attr(dat, "infTravel", NA, nArrivals)
      dat <- append_attr(dat, "secInfs", NA, nArrivals)
      dat <- append_attr(dat, "infGen", NA, nArrivals)
      dat <- append_attr(dat, "age", 21, nArrivals)
      dat <- append_attr(dat, "vaccTime1", NA, nArrivals)
      dat <- append_attr(dat, "vaccTime2", NA, nArrivals)
      dat <- append_attr(dat, "vaccWaneTime_1", NA, nArrivals)
      dat <- append_attr(dat, "vaccWaneTime_2", NA, nArrivals)
      dat <- append_attr(dat, "recTime", NA, nArrivals)
      dat <- append_attr(dat, "waned_1", NA, nArrivals)
      dat <- append_attr(dat, "waned_2", NA, nArrivals)
      dat <- append_attr(dat, "age.grp", 1, nArrivals)
      dat <- append_attr(dat, "sqrt.age", sqrt(21), nArrivals)
      dat <- append_attr(dat, "vax_protect", NA, nArrivals)
      
      # Assign sexual roles 
      assigned_sexual_roles <- sample(c("I", "R", "V"), size=nArrivals, prob=c(0.242, 0.321, 0.437), replace=TRUE)
      dat <- append_attr(dat, "role.class", assigned_sexual_roles, nArrivals)
      
      # Assign risk group 
      assigned_risk_roles <- sample(c(1,2,3,4,5,6), size=nArrivals, prob=c(0.19,0.19,0.19,0.19,0.19,0.05), replace=TRUE)
      dat <- append_attr(dat, "riskg", assigned_risk_roles, nArrivals)
      
      # Assign degree main 
      assigned_deg_main <- sample(c(0,1), size=nArrivals, prob=c(0.712, 0.288), replace=TRUE)
      dat <- append_attr(dat, "deg.main", assigned_deg_main, nArrivals)
      
      # Assign degree pers 
      assigned_deg_pers <- sample(c(0,1,2), size=nArrivals, prob=c(0.691, 0.214, 0.095), replace=TRUE)
      dat <- append_attr(dat, "deg.pers", assigned_deg_pers, nArrivals)
      
      # # Set nodal attribute for tx seeking if nodal.tx=TRUE
      # if (dat$param$nodal.tx){
      #   tx.prob <- get_param(dat, "treatment.prob")
      #   tx.seek <- rep(0, nArrivals)
      #   seek <- sample(1:nArrivals, tx.prob*nArrivals, replace=FALSE)
      #   tx.seek[seek] <- 1
      #   dat <- append_attr(dat, "tx.seek", tx.seek, nArrivals)
      # }
      
      # Race isn't actually being used for fitting purposes, but is a nodal attribute
      # for consistency, we will assign 50% B and 50% W race
      dat <- append_attr(dat, "race", ifelse(rbinom(nArrivals, 1, 0.50), "B", "W"), nArrivals)
      
      # Likewise, apply HIV status, though its not currently being used for any purpose in the model
      dat <- append_attr(dat, "hiv", ifelse(rbinom(nArrivals, 1, 0.8846), 0, 1), nArrivals)
    }
    
    ## Summary statistics ##
    dat <- set_epi(dat, "arrival.flow", at, nArrivals)
    
    ## Update edge list
    dat$el[[1]] <- add_vertices(dat$el[[1]], nArrivals)
    dat$el[[2]] <- add_vertices(dat$el[[2]], nArrivals)
    dat$el[[3]] <- add_vertices(dat$el[[3]], nArrivals)
...
aging <- function(dat, at) {
    ...
    if(dat$param$demography.change == TRUE){    
  ...
  # Removing Older Adults  ------------------------
  
departures <- function(dat, at) {
    ...
    ## Parameters ##
    r.rate <- get_param(dat, "removal.rate")
    
    ## Query those who are alive
    idsElig <- which(active == 1 & age > 40.0)
    ...
    if (nElig > 0) {
      ...
      idsRemoval <- idsElig
      nDepartures <- length(idsRemoval)
      
      if (nDepartures > 0){
        # Update the status and exit time of these nodes that have aged out of the network
        active[idsRemoval] <- 0
        exitTime[idsRemoval] <- at
        
        ## Update edge list
        # dat$el[[1]] <- delete_vertices(dat$el[[1]], idsElig)
        # dat$el[[2]] <- delete_vertices(dat$el[[2]], idsElig)
        # dat$el[[3]] <- delete_vertices(dat$el[[3]], idsElig)
      }
    }
    ...
    dat <- set_epi(dat, "removal.flow", at, nDepartures)
...
vax_msm <- function(dat, at) {
  ...
  if(wane.change==TRUE){
    ...
    nEligV1.adj = round(as.numeric(vd[vd$TIME_STEP == at,'FD_ACTUAL_NORM'])*pop.size, digits=0)
    if (nEligV1 > 0) {
    ...
    nWane_1 = 0     #Initiate with 0 people who are vaccinated with 1 dose and wanining
    ...
    if (nEligWane_1 > 0) {
      for (i in 1:nEligWane_1){    # For loop - goes through each eligible person in the list
        person_1 = idsEligWane_1[i]   # Get the ID of person i and assign to this placeholder variable
        person.time.vac_1 = at-vaccTime1[person_1]  # Calculate the time since first vaccine dose and assign to this placeholder variable
        vecWane_1 = append(vecWane_1, rbinom(1, 1, prob=waning.prob[waning.prob$time == person.time.vac_1, 'prob_out_1']))  # Add to vector of whether or not they are waned
      }
    ...
    nEligV2.adj = round(as.numeric(vd[vd$TIME_STEP == at,'SD_ACTUAL_NORM'])*pop.size, digits=0)
    if (nEligV2 > 0) {
    ...
    if (nEligWane_2 > 0) {
      for (i in 1:nEligWane_2){    # For loop - goes through each eligible person in the list
        person = idsEligWane_2[i]   # Get the ID of person i and assign to this placeholder variable
        person.time.vac = at-vaccTime2[person]  # Calculate the time since first vaccine dose and assign to this placeholder variable
        vecWane_2 = append(vecWane_2, rbinom(1, 1, prob=waning.prob[waning.prob$time == person.time.vac, 'prob_out_2']))  # Add to vector of whether or not they are waned
      }
...
travelers = function(dat, at){
  ...
  num_travelers  <- get_param(dat, "num_travelers")
  ...
  if(travel.change==TRUE){
    ...
    if(at == 420){           # Seeding 10 new infections at PRIDE 2023 NYC
      if (nInfTrv_A > 0) {
        vecInfTrv <-  sample(idsEligInfTrv, num_travelers, replace=FALSE)  #Those who get sampled to become infected
...
infect_msm2 <- function(dat, at) {
  ...
  discord_edgelist_mpx <- function (dat, at, network){ 
    ...
    if (nrow(el) > 0) {
      ...
      #isInf <- matrix(stat %in% c("a"), ncol = 1)
...
progress_msm <- function(dat, at) {
  ...
  if (dat$param$asymp == TRUE) {
    ...
    if (dat$param$reinfect == TRUE) {
      
      prob_reinfect <-  get_param(dat, "risk_reinfection")
      time_to_wane <-  get_param(dat, "time_wane_from_rec")
      
      nRtoS <- 0
      idsEligRecovereds <- which(active == 1 & status == "r" & recTime > time_to_wane)
      nEligRecovereds <- length(idsEligRecovereds)
      
      if (nEligRecovereds > 0) {
        vecRtoS <- which(rbinom(nEligRecovereds, 1, prob_reinfect) == 1) 
        if (length(vecRtoS) > 0) {
          idsWaned <- idsEligRecovereds[vecRtoS]
          nRtoS <- length(idsWaned)
          status[idsWaned] <- "s"
        }
      }
    ...
    if (nEligInf > 0) {
      vecInf <- which(rbinom(nEligInf, 1, e.to.i.rate) == 1) 
      if (length(vecInf) > 0) {
        idsInf <- idsEligInf[vecInf]
        nInf <- length(idsInf)
        status[idsInf] <- "i"
