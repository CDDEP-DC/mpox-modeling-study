...
# Parameters ####
param_msm <- function(init.inf = 0,                                # initial number of infected people 
                      ...
                      inf.prob = 0.7,       # probability of infection upon sexual contact
...
new_adults <- function(dat, at) {
  ...
  if (nArrivals > 0) {
    ...
    dat <- append_attr(dat, "vaccWaneTime", NA, nArrivals)   ## NEED TO ADD VAC WANETIME2
  }
...
old_adults <- function(dat, at) {
  ...
  ## Process ##
  nRemovalsExp <- n * r.rate
  nRemovals <- rpois(1, nRemovalsExp)
  ...
  ## Summary statistics ##
  dat <- set_epi(dat, "removal.flow", at, nRemovals)
...
vax_msm <- function(dat, at) {
  ...
  ### Vaccine Dose 2 --> Waned Immunity  ####
  nWane2 = 0     #Initiate with 0 people who are vaccinated with 1 dose and wanining
  idsEligWane2 = which(active == 1 & status == "v2")  #Go through those who are active and vaccinated in the time step
  nEligWane2 = length(idsEligWane2) #Return the length of this ID
  vecWane2 = c() #Set an empty vector of those who will be waned
  
  if (nEligWane2 > 0) {
    for (i in 1:nEligWane2){    # For loop - goes through each eligible person in the list
      person = idsEligWane2[i]   # Get the ID of person i and assign to this placeholder variable
      person.time.vac = at-vaccTime2[person]  # Calculate the time since first vaccine dose and assign to this placeholder variable
      vecWane2 = append(vecWane2, rbinom(1, 1, prob=waning.prob[waning.prob$time == person.time.vac, 'prob_out']))  # Add to vector of whether or not they are waned
    }
    
    vecWane2 = which(vecWane2 == 1)   # Restrict to those who only are considered waned
    
    if (length(vecWane2) > 0){
      idsWane2 = idsEligWane[vecWane2]
      nWane2 = length(idsWane2)
      status[idsWane2] = "s.wane"   #Assign a status back to susceptible
      waned[idsWane2] = "yes"
      vaccWaneTime[idsWane2] = at
    }
  }
...
infect_msm2 <- function(dat, at) {
  ...
  # Seed initial infections at timestep = 19 days
  if(at==10){
    ids_to_infect = which(active == 1 & status == "s")  #Go through those who are active and susceptible in the time step
    vect_infect = sample(ids_to_infect,2, replace=FALSE)  # Sample 2 people to infect
    ids_to_infect = vect_infect
    nInfectInitial = length(ids_to_infect)
    status[ids_to_infect] = "e"    # Assign I status
    infTime[ids_to_infect] = at    # Assign time of infection
    verbose = TRUE
}
