...
city_to_run <- args[1]
prop_reinfection <- 1 / (as.numeric(args[2]) * 30 )
...
if(city_to_run == "nyc"){
  ...
  obj_name <-  paste0("sim_nyc_", "reinfect_", round(prop_reinfection,6), "_inf_", prob_infection)
  file_path_to_save <- paste0(here("mpox","Output", "New York City", "Reinfection Scenario/"), obj_name, ".RData")
} else if(city_to_run == "chi"){
  load(here("mpox","R", "Model", "vaxDataFit_CHI.Rdata"))
  obj_name <-  paste0("sim_chi_", "reinfect_", round(prop_reinfection,6), "_inf_", prob_infection)
  file_path_to_save <- paste0(here("mpox","Output", "Chicago", "Reinfection Scenario/"), obj_name, ".RData")
} else if(city_to_run == "sf"){
  load(here("mpox","R", "Model", "vaxDataFit_SF.Rdata"))
  obj_name <-  paste0("sim_sf_", "reinfect_", round(prop_reinfection,6), "_inf_", prob_infection)
  file_path_to_save <- paste0(here("mpox","Output", "San Francisco", "Reinfection Scenario/"), obj_name, ".RData")
}
...
reinfections.change.switch = TRUE
