...
reinfection_sf_weekly <- lapply(reinfection_sf, function(df) {
  n_rows <- nrow(df)
  n_weeks <- n_rows %/% 7
...
date_sequence_weekly <- seq.Date(from = as.Date("2022-05-01"), by = "weeks", length.out = nrow(reinfection_sf_weekly[[1]]))

# Apply summary statistics calculation to each data frame in the list
reinfection_sf_stats <- lapply(reinfection_sf_weekly, function(df) {
...
extracted_medians <- lapply(reinfection_sf_stats, function(df) {
  return(df$median)
})

# Convert the list to a data frame
combined_medians_sf_reinfect <- do.call(cbind, extracted_medians)

# Rename the columns to match the names of the original list elements
colnames(combined_medians_sf_reinfect) <- names(reinfection_sf_stats)

# Initialize an empty data frame to store RMSE values
rmse_values_reinfect_sf <- data.frame('ColumnName' = character(ncol(combined_medians_sf_reinfect)),
                          'RMSE' = numeric(ncol(combined_medians_sf_reinfect)),
                          "R2" = numeric(ncol(combined_medians_sf_reinfect)),
                          stringsAsFactors = FALSE)

# Loop through each column of all_medians_7day_ma to calculate RMSE
for(i in 1:ncol(combined_medians_sf_reinfect)) {
  sim_column <- combined_medians_sf_reinfect[, i]
  ...
  rmse_values_reinfect_sf$ColumnName[i] <- colnames(combined_medians_sf_reinfect)[i]
  rmse_values_reinfect_sf$RMSE[i] <-  sqrt(mean((sim_column_overlap - SF_weekly_cases_overlap)^2))

  # Calculate R^2
  model <- lm(SF_weekly_cases_overlap ~ sim_column_overlap)
  rmse_values_reinfect_sf$R2[i] <- summary(model)$r.squared
}

save(rmse_values_reinfect_sf, file = here("Output", "Processed Files", "RMSE_sf_reinfect.Rdata"))
save(combined_medians_sf_reinfect, file = here("Output", "Processed Files", "simulated_cases_weekly_reinfect_sf.Rdata"))
save(reinfection_sf_stats, file = here("Output", "Processed Files", "reinfect_sf_stats_daily.Rdata"))
save(reinfection_sf_weekly, file = here("Output", "Processed Files", "reinfect_sf_stats_weekly.Rdata"))
...
ggsave(here("Output", "Figures", "reinfect_sf.svg"),
       height = 140,
       width = 176,
       units = "px",
       dpi = 72)
