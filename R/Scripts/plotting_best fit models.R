...
ggplot(asymptomatic_chi_stats$as_0.9_inf_0.7, aes(x=date, y=median))+
...
ggsave(here("Output", "Figures", "as_chi_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
...
last_day_nyc = last_days_asy_all %>%
  filter(city == "New York City")

last_day_nyc = last_day_nyc  %>%
  filter(as_value == 0.7 & inf_value == 0.7)
...
ggplot(asymptomatic_nyc_stats$as_0.7_inf_0.7, aes(x=date, y=median))+
  ...
  geom_line(aes(col="Model"),size=0.5)+
  geom_line(aes(x=date, y=mean, col="Model"), linetype = "dotted",size=0.5)+
  geom_point(data = NYC_weekly_data, aes(x=week_starting, y =weekly_scaled_cases_adjusted, col="Observed"), size=0.5, shape = 3)+
...
ggsave(here("Output", "Figures", "as_nyc_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
...
ggsave(here("Output", "Figures", "as_sf_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
