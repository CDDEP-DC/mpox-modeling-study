...
rmse_df = rmse_values_as_nyc %>%
  separate(ColumnName, into = c("asymp", "inf"), sep = "_inf_") %>%
  mutate(asymp = as.numeric(gsub("as_", "", asymp)),
         inf = as.numeric(inf))

highlight_data <- rmse_df[rmse_df$asymp == 0.7 & rmse_df$inf == 0.7, ]

plot_r2_v_rmse = ggplot(rmse_df, aes(y=R2, x=RMSE, col=as.factor(asymp), shape=as.factor(inf)))+
  geom_point(size=1.5) +
...
ggsave(plot = plot_r2_v_rmse,
       filename = here("Output", "Figures", paste0("RMSE", "_asy", "_nyc", ".svg")),
       height = 140.1581,
       width = 233.5968,
...
#ggsave(filename = here("Output", "Figures", paste0("legend", "_asy", ".svg")), legend_only, width = 200, height = 400, units="px", dpi=72)
