...
reinfection_nyc_weekly <- lapply(reinfection_nyc, function(df) {
  n_rows <- nrow(df)
  n_weeks <- n_rows %/% 7
...
date_sequence_weekly <- seq.Date(from = as.Date("2022-05-01"), by = "weeks", length.out = nrow(reinfection_nyc_weekly[[1]]))

# Apply summary statistics calculation to each data frame in the list
reinfection_nyc_stats <- lapply(reinfection_nyc_weekly, function(df) {
...
extracted_medians <- lapply(reinfection_nyc_stats, function(df) {
  return(df$median)
})

# Convert the list to a data frame
combined_medians_nyc_reinfect <- do.call(cbind, extracted_medians)

# Rename the columns to match the names of the original list elements
colnames(combined_medians_nyc_reinfect) <- names(reinfection_nyc_stats)

# Initialize an empty data frame to store RMSE values
rmse_values_reinfect_nyc <- data.frame('ColumnName' = character(ncol(combined_medians_nyc_reinfect)),
                          'RMSE' = numeric(ncol(combined_medians_nyc_reinfect)),
                          "R2" = numeric(ncol(combined_medians_nyc_reinfect)),
                          stringsAsFactors = FALSE)

# Loop through each column of all_medians_7day_ma to calculate RMSE
for(i in 1:ncol(combined_medians_nyc_reinfect)) {
  sim_column <- combined_medians_nyc_reinfect[, i]
  ...
  rmse_values_reinfect_nyc$ColumnName[i] <- colnames(combined_medians_nyc_reinfect)[i]
  rmse_values_reinfect_nyc$RMSE[i] <-  sqrt(mean((sim_column_overlap - NYC_weekly_cases_overlap)^2))
  
  # Calculate R^2
  model <- lm(NYC_weekly_cases_overlap ~ sim_column_overlap)
  rmse_values_reinfect_nyc$R2[i] <- summary(model)$r.squared
}

save(rmse_values_reinfect_nyc, file = here("Output", "Processed Files", "RMSE_nyc_reinfect.Rdata"))
save(combined_medians_nyc_reinfect, file = here("Output", "Processed Files", "simulated_cases_weekly_reinfect_nyc.Rdata"))
save(reinfection_nyc_stats, file = here("Output", "Processed Files", "reinfect_nyc_stats_daily.Rdata"))
save(reinfection_nyc_weekly, file = here("Output", "Processed Files", "reinfect_nyc_stats_weekly.Rdata"))

mypal <- pal_npg("nrc", alpha = 1)(5)

ggplot(reinfection_nyc_stats$reinfect_0.004762_inf_0.9, aes(x=date, y=median))+
...
ggsave(here("Output", "Figures", "reinfect_nyc.svg"),
       height = 140,
       width = 176,
       units = "px",
       dpi = 72)
...
combined_medians_sf_reinfect = data.frame(combined_medians_sf_reinfect)
combined_medians_sf_reinfect$week = seq.Date(from = as.Date("2022-05-01"), by = "weeks", length.out = nrow(reinfection_sf_weekly[[1]]))

combined_medians_sf_reinfect_long = as.data.frame(combined_medians_sf_reinfect) %>%
  pivot_longer(
    cols = 1:77,
    names_to = "model",
    values_to = "value",
...
ggplot(combined_medians_sf_reinfect_long, aes(x=week, y=value, col=model))+
  geom_line() +
  scale_color_viridis_d()

autoplot(as.data.frame(combined_medians_sf_reinfect), facet = NULL) 


rmse_values_reinfect_sf <- rmse_values_reinfect_sf %>%
  mutate(
    "Reinfection" = ifelse(str_detect(ColumnName, "^\\w+_0(?!\\.)"), "0", str_extract(ColumnName, "(?<=_)(\\d+\\.\\d+)(?=_inf_)")),
    "Infection Probability" = str_extract(ColumnName, "(?<=_inf_)(\\d+\\.\\d+)")
  ) %>%
  mutate(Reinfection = as.numeric(Reinfection),
         `Infection Probability` = as.numeric(`Infection Probability`))

ggplot(rmse_values_reinfect_sf, aes(x=`Infection Probability`,y=round((1/Reinfection)/30,0), fill=log(RMSE))) + 
  geom_tile() +
  scale_fill_material("purple") +
  scale_x_continuous(breaks=c(seq(0.3,0.9,0.1)), expand=c(0,0)) +
  scale_y_continuous(breaks=c(seq(6,16,1)), expand=c(0,0)) +
...
ggsave(here("Output", "Figures", "reinfect_sf_RMSE_grid.svg"),
       height = 200,
       width = 250,
       units = "px",
       dpi = 72)