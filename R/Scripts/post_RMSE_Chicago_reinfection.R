...
assign("reinfection_chicago", get("processed_files"))
rm("processed_files")

# Overwrite your list to only contain the 'ei.flow' data frames from each sublist
reinfection_chicago <- lapply(reinfection_chicago, function(x) x[['ei.flow']])

# Convert daily data to weekly
reinfection_chicago_weekly <- lapply(reinfection_chicago, function(df) {
  n_rows <- nrow(df)
  n_weeks <- n_rows %/% 7
...
date_sequence_weekly <- seq.Date(from = as.Date("2022-05-01"), by = "weeks", length.out = nrow(reinfection_chicago_weekly[[1]]))

# Apply summary statistics calculation to each data frame in the list
reinfection_chicago_stats <- lapply(reinfection_chicago_weekly, function(df) {
...
extracted_medians <- lapply(reinfection_chicago_stats, function(df) {
  return(df$median)
})

# Convert the list to a data frame
combined_medians_chicago <- do.call(cbind, extracted_medians)

# Rename the columns to match the names of the original list elements
colnames(combined_medians_chicago) <- names(reinfection_chicago_stats)

# Initialize an empty data frame to store RMSE values
rmse_values_reinfect_chi <- data.frame('ColumnName' = character(ncol(combined_medians_chicago)),
                          'RMSE' = numeric(ncol(combined_medians_chicago)),
                          "R2" = numeric(ncol(combined_medians_chicago)),
                          stringsAsFactors = FALSE)


# Loop through each column of all_medians_7day_ma to calculate RMSE
for(i in 1:ncol(combined_medians_chicago)) {
  sim_column <- combined_medians_chicago[, i]
  ...
  rmse_values_reinfect_chi$ColumnName[i] <- colnames(combined_medians_chicago)[i]
  rmse_values_reinfect_chi$RMSE[i] <-  sqrt(mean((sim_column_overlap - CHI_weekly_cases_overlap)^2))
  
  # Calculate R^2
  model <- lm(CHI_weekly_cases_overlap ~ sim_column_overlap)
  rmse_values_reinfect_chi$R2[i] <- summary(model)$r.squared
}

save(rmse_values_reinfect_chi, file = here("Output", "Processed Files", "RMSE_chi_reinfection.Rdata"))
save(combined_medians_chicago, file = here("Output", "Processed Files", "simulated_cases_weekly_chi_reinfections.Rdata"))
save(reinfection_chicago_stats, file = here("Output", "Processed Files", "reinfect_chicago_stats_daily.Rdata"))
save(reinfection_chicago_weekly, file = here("Output", "Processed Files", "reinfect_chicago_stats_weekly.Rdata"))
...
ggsave(here("Output", "Figures", "reinfect_chicago.svg"),
       height = 140,
       width = 176,
       units = "px",
       dpi = 72)
