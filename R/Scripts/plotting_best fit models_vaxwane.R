load(here("Output", "Processed Files", "vaxwane_chicago_stats_daily.Rdata"))
load(here("Output", "Processed Files", "vaxwane_nyc_stats_daily.Rdata"))
load(here("Output", "Processed Files", "vaxwane_sf_stats_daily.Rdata"))
...
ggsave(here("Output", "Figures", "vw_chi_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
...
last_day_nyc = last_days_vw_nyc  %>%
  filter(wP == 6 & wF == 9 & inf_value == 0.8)
...
ggplot(vaxwane_nyc_stats$wP_6_wF_9_inf_0.8, aes(x=date, y=median))+
  geom_ribbon(aes(ymin=perc2.5, ymax=perc97.5, fill="Model_95"), alpha=0.50)+
  geom_ribbon(aes(ymin=perc25, ymax=perc75, fill="Model_IQR"), alpha=0.50)+
  geom_line(aes(col="Model"),size=0.5)+
  geom_line(aes(x=date, y=mean, col="Model"), linetype = "dotted",size=0.5)+
...
ggsave(here("Output", "Figures", "vw_nyc_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
...
ggsave(here("Output", "Figures", "vw_sf_new.svg"),
       height = 128.2711,
       width = 195.849,
       units = "px",
       dpi = 72)
