...
asymptomatic_chi_weekly <- lapply(asymptomatic_chi, function(df) {
  n_rows <- nrow(df)
  n_weeks <- n_rows %/% 7
...
date_sequence_weekly <- seq.Date(from = as.Date("2022-05-01"), by = "weeks", length.out = nrow(asymptomatic_chi_weekly[[1]]))

# Apply summary statistics calculation to each data frame in the list
asymptomatic_chi_stats <- lapply(asymptomatic_chi_weekly, function(df) {
...
extracted_medians <- lapply(asymptomatic_chi_stats, function(df) {
  return(df$median)
})

# Convert the list to a data frame
combined_medians_chi_asymp <- do.call(cbind, extracted_medians)

# Rename the columns to match the names of the original list elements
colnames(combined_medians_chi_asymp) <- names(asymptomatic_chi_stats)

# Initialize an empty data frame to store RMSE values
rmse_values_as_chi <- data.frame('ColumnName' = character(ncol(combined_medians_chi_asymp)),
                          'RMSE' = numeric(ncol(combined_medians_chi_asymp)),
                          "R2" = numeric(ncol(combined_medians_chi_asymp)),
                          stringsAsFactors = FALSE)

# Loop through each column of all_medians_7day_ma to calculate RMSE
for(i in 1:ncol(combined_medians_chi_asymp)) {
  sim_column <- combined_medians_chi_asymp[, i]
  ...
  rmse_values_as_chi$ColumnName[i] <- colnames(combined_medians_chi_asymp)[i]
  rmse_values_as_chi$RMSE[i] <-  sqrt(mean((sim_column_overlap - CHI_weekly_cases_overlap)^2))
  
  # Calculate R^2
  model <- lm(CHI_weekly_cases_overlap ~ sim_column_overlap)
  rmse_values_as_chi$R2[i] <- summary(model)$r.squared
}

save(rmse_values_as_chi, file = here("Output", "Processed Files", "RMSE_chi_as.Rdata"))
save(combined_medians_chi_asymp, file = here("Output", "Processed Files", "simulated_cases_weekly_chi.Rdata"))
save(asymptomatic_chi_stats, file = here("Output", "Processed Files", "as_chi_stats_daily.Rdata"))
save(asymptomatic_chi_weekly, file = here("Output", "Processed Files", "as_chi_stats_weekly.Rdata"))

mypal <- pal_npg("nrc", alpha = 1)(5)


ggplot(asymptomatic_chicago_stats$as_0.9_inf_0.7, aes(x=date, y=median))+
...
ggsave(here("Output", "Figures", "as_chi.svg"),
       height = 140,
       width = 176,
       units = "px",
       dpi = 72)
